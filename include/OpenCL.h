...
public:
  ...
  Context(std::string const &vendorName, bool verbose = true) {
...
public:
  ...
  Buffer(Context const &context, IteratorType begin, IteratorType end)
    ...
    switch (access) {
    case Access::read:
      flags = CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY;
      break;
    case Access::readWrite:
    default:
      flags = CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE;
    }
  ...
  Buffer(Context const &context, size_t nElements)
    ...
    devicePtr_ = clCreateBuffer(context_.context(), flags,
                                sizeof(T) * nElements_, nullptr, &errorCode);
    if (errorCode != CL_SUCCESS) {
  ...
  Buffer<T, access> &operator=(Buffer<T, access> &&) = delete;
...
public:
  ...
  double ExecuteTask() {
    cl_event event;
    auto errorCode =
        clEnqueueTask(context_.commandQueue(), kernel_, 0, nullptr, &event);
    if (errorCode != CL_SUCCESS) {
...
Buffer<T, access> Context::MakeBuffer(Ts... args) {
  return Buffer<T, access>(*this, args...);
}
